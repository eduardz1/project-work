...
def validate_solution(
    ...
    for i in range(len(solution) - 1):
        ...
        if city_from == city_to:
            ...
            if abs(weight_to - weight_from) > 1e-4:
                errors.append(
            ...
            if abs(weight_to - weight_from) < 1e-4:
                ...
                pass
            elif abs(weight_to - weight_from - gold_at_dest) < 1e-4:
                ...
                # This is actually valid - partial gold collection
            else:
                errors.append(
    ...
    for node in G.nodes:
        ...
        # Use larger tolerance for floating point accumulation errors
        if diff > 1e-4:
            errors.append(
                f"Node {node}: Expected {expected:.2f} gold, but collected {collected:.2f} (diff: {diff})"
            )
